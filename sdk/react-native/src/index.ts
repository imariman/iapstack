...
import type { IapStackConfig } from './config';
import {
  IapStackApiError,
...
export { IapStackConfig } from './config';
export * from './config';
export * from './errors';
...
export class IapStackClient {
    ...
    for (let attempt = 1; attempt <= this.retryPolicy.maxAttempts; attempt++) {
      try {
        ...
        if (
          ...
          await delay(
            this.retryPolicy.delayAfter(attempt, Math.random()),
          );
          continue;
        }
        if (
  ...
  private async _attempt(
    ...
    try {
      ...
      const domMessage = error && typeof error === 'object' && 'name' in error ? `${(error as { name?: unknown }).name}` : '';
  ...
  private _buildUri(pathSegments: string[]): URL {
    ...
    const appendedPath = pathSegments.map(encodeURIComponent).join('/');
