...
| --- | --- | --- | --- |
| T01 | Stolen or guessed administrator/application bearer, dashboard session, or customer session | Generated keys contain 256-bit random secrets, only Argon2id verifiers are stored, bootstrap comparison is constant-time, bootstrap keys shorter than 32 bytes are rejected, and every route enforces role and application scope. Customer sessions are encrypted, customer-bound, expire after 15 minutes, and require a still-active issuer key. Dashboard sessions are encrypted, expire after 12 hours, remain inaccessible to JavaScript in a Secure HttpOnly cookie, and re-check stored issuer-key revocation. Stored-key listing is secret-free; revocation is idempotent and atomically preserves the final active administrator. | TLS and ingress rate limits are mandatory. Keep durable application bearers in trusted host backends, remove the bootstrap key after provisioning, and rotate through a replacement bearer before revoking the old key. A bootstrap-issued dashboard session remains valid only until its bounded expiry because bootstrap credentials have no revocable stored identity. |
...
| T03 | Outbound SSRF, DNS rebinding, or redirect escape | Webhook and configurable Huawei provider endpoints accept only absolute HTTPS URLs without user information or fragments. Every A/AAAA result is validated at connection time, the connection is pinned to a validated IP, mixed public/private answers are rejected by default, proxies and redirects are disabled, and response size/time are bounded. | Private-network webhook and Huawei provider access are independently disabled by default. When either is explicitly enabled, infrastructure egress allowlisting becomes mandatory because the corresponding address blocking is intentionally bypassed. |
| T04 | Forged, cross-customer, or replayed provider notification | Huawei subscription signatures are verified before acknowledgement. Legacy V2 ORDER callbacks have no equivalent signature, so the protected token fingerprint must resolve to a previously verified purchase with the same application, product, and kind in the same transaction that inserts the inbox row. Google Play pushes require a Google-signed OIDC token with exact audience and verified service-account email, exact Pub/Sub subscription and package scope, and one strict RTDN variant. Strict normalized envelopes share one protected inbox identity. Workers query authoritative provider state; unknown notification tokens fail permanently and cannot change access. | Apply ingress limits to bound repeated valid evidence. A leaked Huawei purchase token, compromised Huawei signing material, or a compromised configured Google push principal is outside this boundary. |
...
| T06 | Webhook payload tampering or receiver replay | Each immutable outbox body is signed with HMAC-SHA-256 over the versioned `v2` MAC input `v2\\n<event_id>\\n<timestamp>\\n<raw_body>`. Redirects, response size, attempts, and retryable statuses are bounded. | Receivers must verify the raw body and authenticated event ID, reject `v1` signatures, enforce a timestamp tolerance, and atomically deduplicate event IDs. Delivery is at-least-once, not exactly-once. |
...
| T09 | Secret or purchase leakage through logs, metrics, errors, or dashboard | Access logs use route patterns and bounded request IDs; provider and worker failures expose stable codes; metrics use bounded labels and allow-list HTTP methods with one `OTHER` bucket; `/metrics` is disabled without a dedicated bearer and verifies configured credentials in constant time; direct OTLP export requires HTTPS and a write-only secret; admin reads omit ciphertext, fingerprints, secrets, tokens, and payloads; JSON responses are `no-store`. | Restrict Render logs and Grafana membership to administrators, keep Grafana anonymous/public sharing disabled, and do not add user/provider values as metric labels or error text. |
...
| T11 | Dashboard session theft, CSRF, or browser injection | The dashboard exchanges the administrator bearer once, clears the input, and uses an encrypted 12-hour cookie with `Secure`, `HttpOnly`, `SameSite=Strict`, an admin-only path, and same-origin mutation proof. It also applies `no-store`, a restrictive CSP, frame denial, MIME sniffing denial, and no third-party scripts. | Use a trusted, patched browser over TLS. Same-origin script execution or a hostile browser extension can still act with the session even though it cannot read the cookie; use a dedicated administration origin where possible. |
...
6. Notification acknowledgement occurs only after provider signature validation or an
   explicitly enforced application-scoped purchase-reference authentication, followed by
...
- [ ] `IAPSTACK_HUAWEI_ALLOW_PRIVATE_NETWORKS` is `false`, or an approved egress
  allowlist documents every permitted internal provider destination.
