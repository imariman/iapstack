...
| T04 | Forged, cross-customer, or replayed Huawei notification | The signed purchase bytes, application, product, and developer-payload customer binding are verified before acknowledgement. Strict envelopes are normalized before fingerprinting, so harmless JSON ordering/whitespace changes share one inbox identity. Workers query authoritative Huawei state instead of trusting lifecycle claims. | Apply ingress limits to bound repeated valid evidence. A compromised Huawei key is outside this boundary. |
...
| T10 | Slow request, oversized input/output, or worker exhaustion | HTTP header/body/read/write/idle limits, provider and webhook timeouts, bounded provider/webhook responses, worker concurrency, per-job deadlines, and maximum attempts constrain resource use. | A distributed rate limiter and volumetric protection remain ingress responsibilities in v0.1. |
